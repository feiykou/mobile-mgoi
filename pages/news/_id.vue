<template>
    ...
    <thumb class="font12">
      <thumb-item><nuxt-link to="/about" hover-class="hover-class">新闻列表</nuxt-link></thumb-item>
      <thumb-item>新闻详情</thumb-item>
    </thumb>
        ...
        <!-- <div class="sub_name font15">{{detailData.sub_name}}</div> -->
			...
			<div class="section-wrap">
					...
					<div class="page-wrap" v-if="topBottomData">
						<div class="page">
							<span>上一篇：</span>
              <nuxt-link v-if="topBottomData.prev_info" :to="`/news/${topBottomData.prev_info.id}`">{{ topBottomData.prev_info.name }}</nuxt-link>
							<i v-else>没有了</i>
						</div>
						<div class="page">
							<span>下一篇：</span>
              <nuxt-link v-if="topBottomData.next_info" :to="`/news/${topBottomData.next_info.id}`">{{ topBottomData.next_info.name }}</nuxt-link>
							<i v-else>没有了</i>
						</div>
					</div>
			</div>
...
export default {
    ...
    async asyncData({ params, error }) {
        const id = Number(params.id),
            detailData = await newsModel.getNewsDetail(id)
