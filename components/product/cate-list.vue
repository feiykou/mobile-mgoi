    ...
    <van-popup 
      ...
      <div class="cate-wrap">
        <div class="cate-list-wrap" v-for="(item, index) in categoryCate" :key="index">
          <m-cell v-if="!item.sub" :url="item.link_url" :name="item.name" @cellTap="onCellTap"></m-cell>
          <div class="group-wrap" v-if="item.sub">
            <div class="divider-tit font15 bold" @click="onCellTap"><nuxt-link :to="item.link_url">{{ item.name }}</nuxt-link></div>
            <div v-for="(sItem, sIndex) in item.sub" :key="sIndex">
              <m-cell v-if="!sItem.sub" :url="sItem.link_url" :name="sItem.name" :noLine="sIndex===Object.keys(item.sub).pop()"></m-cell>
              <van-collapse v-if="sItem.sub" class="cate-collapse-wrap" v-model="activeNames" @change="colChange">
                <van-collapse-item class="cate-collapse-item" :title="sItem.name" :name="sIndex">
                  ...
                  <m-cell :url="sItem.link_url" :name="sItem.name+'全部商品'" v-hover-class="'hover-class'" @cellTap="onCellTap"></m-cell>
                  <m-cell v-for="(ssItem,ssIndex) in sItem.sub" :url="ssItem.link_url" :noLine="ssIndex===Object.keys(sItem.sub).pop()" :key="ssIndex" :name="'- ' + ssItem.name" v-hover-class="'hover-class'" @cellTap="onCellTap"></m-cell>
                </van-collapse-item>
              </van-collapse>
            </div>
        ...
        <div class="group-wrap web-nav-wrap">
          <m-cell name="唔该专栏" @cellTap="onCellTap"></m-cell>
          <m-cell name="唔该新闻" @cellTap="onCellTap"></m-cell>
          <m-cell name="唔该招聘" :noLine="true" @cellTap="onCellTap"></m-cell>
        </div>
